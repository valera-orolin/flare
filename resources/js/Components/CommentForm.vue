...
<template>
    <div class="sticky bottom-0 z-20 pb-6 bg-white">
        <div class="w-full bg-blue-100 rounded-lg p-6 space-y-2">
            <form @submit.prevent="form.post(route('posts.comments.store', post.id), { onSuccess: () => form.reset() })">
                <textarea id="post-area" maxlength="1000" v-model="form.message"
